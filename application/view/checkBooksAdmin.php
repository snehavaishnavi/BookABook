...
<script>
	...
	function loadBooks(subId){
		$('#booksTable').html("");
		$.get('index.php?op=getBooksViewA&sub=' + subId, function(data) {
			$('#booksTable').html(data);
			$('#example-datatableBook').DataTable();
		});
	}
        function loadBookDetails(bookId){
            $('#bookDetailsTable').html("");
            $.get('index.php?op=getBookDetailsView&bookId=' + bookId, function(data) {
			$('#bookDetailsTable').html(data);
			$('#example-datatableBookDetails').DataTable();
                        window.location = "index.php?op=loanSuccess";
		});
        }
        function updateLoanA(bookId){
		$('#booksTable').html("");
		$.get('index.php?op=getUpdateLoanA&bookId=' + bookId, function(data) {
			//$('#booksTable').html(data);
                        window.location = "index.php?op=atHome1";
			//$('#example-datatableBook').DataTable();
		});
	}
